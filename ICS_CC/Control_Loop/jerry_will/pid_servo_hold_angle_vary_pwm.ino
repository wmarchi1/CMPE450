...
// Holds a fixed servo angle and ramps yVal from yValSlow(450) to yValFast(100)
// over rampMs, then back to yValSlow, then stops. Runs once.
// set_control_params() + drive() in loop() handle actual motor output.
// Logs CSV rows (servo_angle,yVal,heading,gyro_z,vel) at 20 Hz.
//
// Usage — call in loop() alongside drive():
//   vel_sweep_test(90, 450, 100, 5000);   // center steer, slow→fast→slow over 5 s each
void vel_sweep_test(int fixedAngle, int yValSlow, int yValFast, unsigned long rampMs) {
  static uint8_t       phase      = 0;  // 0=ramp up  1=ramp down  2=done
  ...
  if (!running) {
    ...
    myservo.write(fixedAngle);
    servoAngle = fixedAngle;
    xVal = 512;  // straight — differential drive stays symmetric

    Serial.println("servo_angle,yVal,heading,gyro_z,vel");
  }

  unsigned long now     = millis();
  unsigned long elapsed = now - phaseStart;
  float t = constrain((float)elapsed / (float)rampMs, 0.0f, 1.0f);

  if (phase == 0) {
    // ramp up: yValSlow → yValFast
    yVal = (int)(yValSlow + t * (yValFast - yValSlow));
    if (elapsed >= rampMs) { phase = 1; phaseStart = now; }
  } else {
    // ramp down: yValFast → yValSlow
    yVal = (int)(yValFast + t * (yValSlow - yValFast));
    if (elapsed >= rampMs) {
      yVal = 512;  // neutral
      myservo.write(90);
      servoAngle = 90;
  ...
  // Keep servo locked (set_control_params won't touch it while flag=false)
  myservo.write(fixedAngle);
  servoAngle = fixedAngle;
  ...
  if (now - lastLog >= 50) {
    lastLog = now;
    Serial.print(fixedAngle);        Serial.print(',');
    Serial.print(yVal);              Serial.print(',');
    Serial.print(vehicle_heading, 2); Serial.print(',');
    Serial.print(gyro_z, 2);         Serial.print(',');
    Serial.println(avgVel, 3);
  }
}
...
void loop() {
  ...
  if(flag);
  drive();
  vel_sweep_test(90, 450, 100, 5000);
