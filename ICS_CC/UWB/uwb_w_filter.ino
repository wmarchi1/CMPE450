...
float applyKalman2(float measurement) {
    // Prediction update
    kalmanP = kalmanP + kalmanQ;
...
void loop() {
    ...
    if (uwb.receiveMessage(info)) {
        float filteredDistance2 = applyKalman2(info.distance);
        ...
        Serial.println("Anchor did not receive a response from tag1");
        delay(500);
    }
    
    delay(100);
    uwb.sendMessage("TAG2", "TEST_MSG");
    ...
    if (uwb.receiveMessage(info)) {
        ...
        Serial.println("Anchor did not receive a response from tag2");
        delay(500);
    }
